...
// --- CSRF TOKEN (pour requêtes POST sécurisées) ---
...
// Au cas où la page recharge dynamiquement
document.addEventListener("DOMContentLoaded", sendCSRFToBackground);
...
function groupByPlayer(carto) {
  ...
  carto.forEach(p => {
    ...
    players[p.owner].planets.add(formatCoords(p.galaxy, p.system, p.planet));
  });
...
function formatCoords(galaxy, system, planet) {
  const g = String(galaxy).padStart(1, '0');
  const s = String(system).padStart(3, '0');
  const p = String(planet).padStart(2, '0');
  return `${g}-${s}-${p}`;
}
